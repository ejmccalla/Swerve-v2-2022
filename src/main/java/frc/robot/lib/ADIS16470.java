...
public class ADIS16470 implements AutoCloseable, NTSendable {
  ...
  private static class AcquireTask implements Runnable {
    private ADIS16470 imu;

    public AcquireTask(ADIS16470 imu) {
      this.imu = imu;
    }
  ...
  public ADIS16470(IMUAxis yaw_axis, SPI.Port port, CalibrationTime cal_time) {
    ...
    if (m_simDevice == null) {
      ...
      /** These are the default settings from WPILib 2022.4.1
       * 
    ...
    //   writeRegister(FILT_CTRL, 0);
      ...
      /**
       * These settings provide the best in-run stability when the CBE is used to
    ...
    while (true) {
      ...
      if (m_thread_active) {
        ...
        for (int i = 0; i < data_to_read; i += dataset_len) {
          ...
          //accel_z = (toShort(buffer[i + 17], buffer[i + 18]) / 800.0); EJM
